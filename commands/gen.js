...
module.exports = {
	...
	description: 'Generates a specified stock', // Command description
        ...
        try {
            ...
            if (config.command.error_message === true) {
                return message.channel.send(
                    ...
                    .setTitle('Error!')
                    .setDescription('Not a valid gen channel')
                    ...
                    if (!error) {
                        ...
                        if (position === -1) {
                            return message.channel.send(
                                ...
                                .setDescription(`I do not find the \`${args[0]}\` items in my stock.`)
                                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                .setTimestamp()
                            );
                        };
                        ...
                        message.author.send(
                            ...
                            .addField('stock', `\`\`\`${args[0][0].toUpperCase()}${args[0].slice(1).toLowerCase()}\`\`\``, true)
                        ...
                        if (position !== -1) {
                            ...
                            fs.writeFile(filePath, data, function (error) {
                                message.channel.send(
                                    ...
                                    .setDescription(`Check your dm's ${message.author}! *If you have not recived the dm please make sure your dm's are open!*`)
                                    .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                    .setTimestamp()
                                );
                            ...
                            return message.channel.send(
                                ...
                                .setColor(config.color.red)
                                .setTitle('Generator error!')
                                .setDescription(`The \`${args[0]}\` stocks are empty!!!`)
                                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                .setTimestamp()
                            );
                        };
